...
import com.kuaidaoresume.kdr.core.interceptor.PostForwardResponseInterceptor;
...
public class RequestForwarder {
  ...
  public ResponseEntity<byte[]> forwardHttpRequest(RequestData data, String traceId, MappingProperties mapping) {
    ...
    RequestEntity<byte[]> request =
      new RequestEntity<>(data.getBody(), data.getHeaders(), data.getMethod(), destination.getUri());
    ...
    log.debug(String.format("Forwarded: %s %s %s -> %s %d", data.getMethod(), data.getHost(),
      data.getUri(), destination.getUri(), response.getStatus().value()));
  ...
  protected URI createDestinationUrl(String uri, MappingProperties mapping) {
    ...
    try {
      ...
      throw new KdrException("Error creating destination URL from HTTP request URI: " + uri + " using mapping " + mapping, e);
    }
  }
  ...
  protected String resolveMetricsName(MappingProperties mapping) {
    return kdrProperties.getMetrics().getNamesPrefix() + "." + mapping.getName();
  }
}
