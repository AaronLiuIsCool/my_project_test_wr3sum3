package com.kuaidaoresume.kdr.spec

import spock.lang.Unroll
import com.kuaidaoresume.kdr.BasicSpec

import static com.kuaidaoresume.kdr.assertions.Assertions.assertThat
...
class ProxyingRequestSpec extends BasicSpec {
    ...
    def "Should proxy HTTP request preserving request method when method is #method"() {
        when:
        sendRequest method, 'www.kuaidaoresume-v2.local', '/path/1'
    ...
    def "Should not proxy HTTP request preserving request method when method is OPTIONS"() {
        when:
        def response = sendRequest method, 'www.kuaidaoresume-v2.local', '/path/1'
    ...
    def "Should proxy HTTP request to #destinationUrl when request uri is #requestUri"() {
        when:
        sendRequest GET, 'www.kuaidaoresume-v2.local', requestUri
    ...
    def "Should proxy HTTP request with headers #destinationHeaders when request headers are #requestHeaders including X-Forwarded headers"() {
        when:
        sendRequest GET, 'www.kuaidaoresume-v2.local', '/path/1', requestHeaders
        ...
        assertThat(localhost8086, localhost8087)
                ...
                .withHeaders(['X-Forwarded-Proto': 'http', 'X-Forwarded-Host': 'www.kuaidaoresume-v2.local', 'X-Forwarded-Port': port.toString()])
                .withoutBody()
    ...
    def "Should proxy HTTP request with headers #destinationHeaders when request headers are #requestHeaders"() {
        ...
        when:
        sendRequest GET, 'ical.kuaidaoresume-v2.local', '/path/9', requestHeaders
        ...
        [:]                                       | ['Host': 'ical.kuaidaoresume-v2.local'] | []
        ['TE': 'compress']                        | ['Host': 'ical.kuaidaoresume-v2.local'] | ['TE']
    }

    @Unroll
    def "Should proxy HTTP request preserving request body when body is '#body'"() {
        when:
        sendRequest POST, 'www.kuaidaoresume-v2.local', '/path/1', [:], body
    ...
    def "Should fail to proxy HTTP request when destination URL cannot be created"() {
        when:
        def response = sendRequest GET, "kdr.kuaidaoresume-v2.local", "/path/4"
