...
export function validateProject(data) {
  return Object.keys(data).every((key) =>
    validateProjectEntry(key, data[key], data),
  );
}
...
const reducers = {
  ...
  updateProjectFromResumeSDK: (state, action) => {
    updateField(
      ...
      'projectRole',
      action.payload.data.proj_name,
    );
    updateField(
      state,
      action.payload.index,
      'projectCompanyName',
      action.payload.data.proj_cpy,
    );
    updateField(
      state,
      action.payload.index,
    ...
    updateField(
      ...
      action.payload.index,
      'projectDescription',
      action.payload.data.proj_content,
    );
  ...
  appendProjectDescription: (state, action) => {
    ...
    const value =
      state.project.data[index].projectDescription.length === 0
        ? action.payload.value
        : `${state.project.data[index].projectDescription}\n${action.payload.value}`;
